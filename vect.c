...
void** append_vect(void** vector, const size_t size, void** elements, const size_t elements_size, size_t* new_size) {
	if (vector == NULL) {
		printf("parameter vector is null!\n");
		return NULL;
	}
	...
	void** result_vector = (void**)malloc((size * elements_size) * sizeof(void*));
